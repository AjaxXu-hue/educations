...
public class UnionPayClient {
    ...
    public static String pay(String orderId,String txnAmt,String goodsName){
        ...
        System.out.println("encoding"+UnionpayBese.encoding);

        /***银联全渠道系统，产品参数，除了encoding自行选择外其他不需修改***/
        requestData.put("version", "5.1.0");   			  //版本号，全渠道默认值
        requestData.put("encoding", UnionpayBese.encoding); 			  //字符集编码，可以使用UTF-8,GBK两种方式
        ...
        /***商户接入参数***/
        requestData.put("merId", UnionpayConfig.MER_ID);    	          			  //商户号码，请改成自己申请的正式商户号或者open上注册得来的777测试商户号
        ...
        Map<String, String> submitFromData = AcpService.sign(requestData,UnionpayBese.encoding);  //报文中certId,signature的值是在signData方法中获取并自动赋值的，只要证书配置正确即可。

        String requestFrontUrl = SDKConfig.getConfig().getFrontRequestUrl();  //获取请求银联的前台地址：对应属性文件acp_sdk.properties文件中的acpsdk.frontTransUrl
        String html = AcpService.createAutoFormHtml(requestFrontUrl, submitFromData,UnionpayBese.encoding);   //生成自动跳转的Html表单
    ...
    public static Map<String, String> query(String orderId,String txnTime){
        ...
        Map<String, String> reqData = AcpService.sign(data,UnionpayBese.encoding);//报文中certId,signature的值是在signData方法中获取并自动赋值的，只要证书配置正确即可。
    ...
    public static Map<String, String> refund(String refundOrderId,String txnAmt,String origQryId){
        ...
        Map<String, String> rspData = AcpService.post(reqData,url,UnionpayBese.encoding);//这里调用signData之后，调用submitUrl之前不能对submitFromData中的键值对做任何修改，如果修改会导致验签不通过
